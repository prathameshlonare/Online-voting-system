################################################################################
# Lambda Function: stopElection
# Description: Stops the election (changes status to STOPPED)
# Trigger: API Gateway POST /election/stop
# Auth Required: Admin Only
# Table: DynamoDB - Config
# Note: Uses conditional update to ensure election is RUNNING before stopping
################################################################################
# stopElection Lambda
import json
import boto3
...
try:
    ...
    logger.error(f"Error initializing Config table in stopElection: {e}")
...
try:
    if COGNITO_REGION and USER_POOL_ID:
        ...
        logger.info(f"Successfully fetched {len(cognito_keys)} Cognito public keys for stopElection.")
    else: logger.warning("Cognito env vars not set for stopElection key fetching.")
except Exception as e: logger.error(f"Failed to fetch Cognito public keys for stopElection: {e}", exc_info=True)
...
def validate_admin_token(token_value):
    ...
    logger.info(f"Admin user {claims.get('cognito:username')} validated successfully (stopElection).")
    return claims


def lambda_handler(event, context):
    """
    Main Lambda handler - Stops the election
    Uses conditional update to ensure election is RUNNING
    """
    logger.info(f"Received event for stopElection: {json.dumps(event, indent=2)}")

    if not config_table or not cognito_keys:
        logger.error("Config table or Cognito keys not initialized in stopElection handler.")
    ...
    try:
        ...
        # Conditional update - only if currently RUNNING
        ...
        logger.info(f"Election status updated to STOPPED: {response}")
        return {
            'statusCode': 200,
            ...
            'body': json.dumps({'message': 'Election stopped successfully.', 'newStatus': 'STOPPED'})
        }
    except dynamodb.meta.client.exceptions.ConditionalCheckFailedException:
        logger.warning("ConditionalCheckFailed: Election was not running (stopElection).")
        return {'statusCode': 409, 'headers': {'Access-Control-Allow-Origin': '*'}, 'body': json.dumps({'error': 'Election is not currently running or in a state that prevents stopping.'})} # 409 Conflict
    except ValueError as ve:
        logger.error(f"Auth Value Error in stopElection: {ve}")
        ...
        logger.error(f"JWT Error in stopElection: {jwt_e}")
        ...
        logger.error(f"DynamoDB ClientError in stopElection: {ce.response['Error']['Message']}", exc_info=True)
        ...
        logger.error(f"Error stopping election: {e}", exc_info=True)
        return {'statusCode': 500, 'headers': {'Access-Control-Allow-Origin': '*'}, 'body': json.dumps({'error': f'Failed to stop election: {str(e)}'})}
